...
def _init_kv_cache(decoder_sess: ort.InferenceSession) -> dict:
    ...
    KV cache tensors have shape (batch, num_kv_heads, seq_len, head_dim).
    The seq_len dimension is dynamic (None); all others are static integers.
    We initialise seq_len=0 so the prefill pass starts with an empty cache.
    """
    past_kv: dict = {}
