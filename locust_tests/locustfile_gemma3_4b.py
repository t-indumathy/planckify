"""Locust load test — Gemma 3 4B IT int8 LiteRT-LM CPU flavour.
...
Run (100 sequential prompts, 1 virtual user — via run_locust_tests.py):
    python run_locust_tests.py --flavour gemma3 --iterations 100

Or directly:
    locust -f locustfile_gemma3_4b.py --headless -u 1 -r 1 -t 8h
           --csv results/gemma3_4b --loglevel WARNING

Outputs under results/:
    gemma3_4b_stats.csv          - Locust aggregate stats (avg, p50, p95, p99, RPS)
    gemma3_4b_stats_history.csv  - time-series throughput and latency
    gemma3_4b_failures.csv       - any failed requests
    gemma3_4b_detailed.csv       - per-request: ttft_s, gen_time_s, tokens, tok/s
"""
...
_EXP_DIR = Path(__file__).resolve().parent.parent / "experiments" / "gemma3_4b_litertlm"
...
_MODEL_PATH = _EXP_DIR / "models" / "gemma3-4b-it-int8-web.task"
...
_DETAILED_CSV = RESULTS_DIR / "gemma3_4b_detailed.csv"
_FIELDNAMES = [
    "seq",
    "prompt_snippet",
    "ttft_s",
...
# ── Locust User ───────────────────────────────────────────────────────────────
class Gemma3CpuUser(User):
    """Single-user sequential load test — Gemma 3 4B IT LiteRT-LM CPU backend."""
    ...
    def on_start(self) -> None:
        ...
        if not _MODEL_PATH.exists():
            raise FileNotFoundError(
                f"[gemma3] Model not found: {_MODEL_PATH}\n"
                "Run experiments/gemma3_4b_litertlm/download_model.py first."
            )
        print(f"\n[gemma3] Loading model from {_MODEL_PATH} …")
        ...
        print(f"[gemma3] Model ready — load time: {time.perf_counter() - t0:.2f}s\n")
