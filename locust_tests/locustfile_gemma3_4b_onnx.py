"""Locust load test — Gemma 3 4B IT int8 raw ONNX Runtime CPU flavour.

ONNX sessions (embed_tokens_quantized + decoder_model_merged_quantized) are
loaded ONCE per virtual user in on_start() and reused across all iterations.
...
Run (100 sequential prompts, 1 virtual user — via run_locust_tests.py):
    python run_locust_tests.py --flavour gemma3_onnx --iterations 100

Or directly:
    locust -f locustfile_gemma3_4b_onnx.py --headless -u 1 -r 1 -t 8h
           --csv results/gemma3_4b_onnx --loglevel WARNING

Outputs under results/:
    gemma3_4b_onnx_stats.csv     - Locust aggregate stats
    gemma3_4b_onnx_detailed.csv  - per-request: gen_time_s, tokens_generated, tok/s
"""
...
_EXP_DIR = Path(__file__).resolve().parent.parent / "experiments" / "gemma3_4b_onnx"
...
_MODEL_DIR = _EXP_DIR / "models" / "gemma-3-4b-it-ONNX"
...
_DETAILED_CSV = RESULTS_DIR / "gemma3_4b_onnx_detailed.csv"
_FIELDNAMES = [
    "seq",
    "prompt_snippet",
    "gen_time_s",
...
# ── Locust User ───────────────────────────────────────────────────────────────
class Gemma3OnnxUser(User):
    """Single-user sequential load test — Gemma 3 4B IT raw ONNX Runtime CPU."""
    ...
    def on_start(self) -> None:
        ...
        if not _MODEL_DIR.exists():
            raise FileNotFoundError(
                f"[gemma3_onnx] Model directory not found: {_MODEL_DIR}\n"
                "Run experiments/gemma3_4b_onnx/download_model.py first."
            )

        embed_path = _MODEL_DIR / "onnx" / "embed_tokens_quantized.onnx"
        decoder_path = _MODEL_DIR / "onnx" / "decoder_model_merged_quantized.onnx"
        if not embed_path.exists() or not decoder_path.exists():
            raise FileNotFoundError(
                f"[gemma3_onnx] ONNX files missing under {_MODEL_DIR}/onnx/.\n"
                "Expected: embed_tokens_quantized.onnx, decoder_model_merged_quantized.onnx"
            )

        print(f"\n[gemma3_onnx] Loading sessions from {_MODEL_DIR} …")
        ...
        print(f"[gemma3_onnx] Sessions ready — load time: {time.perf_counter() - t0:.2f}s\n")
